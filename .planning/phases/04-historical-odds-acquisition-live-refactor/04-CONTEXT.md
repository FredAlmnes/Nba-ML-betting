...
- **D-02:** Fetch **both** the morning-of bet-time snapshot AND the closing-line snapshot for every archived event, in this same paid month — not bet-time only. Rationale: marginal cost is ~1 extra credit/event on top of the ~2 credits/event (1 discovery + 1 fetch) already being spent, since the event is already being paid for; doing it now avoids a second paid month later when Phase 5's CLV requirement (BT-06) needs closing-line data anyway. Confirmed math: for the ~3,638 games in `nba_features.csv` (2022-10-24 → 2025-04-13), bet-time-only ≈ 7,300 credits vs. bet-time+closing ≈ 10,900 credits — both comfortably inside the 20,000-credit budget, leaving headroom for retries on an interrupted fetch.
- **D-03:** Use the **per-event** historical odds endpoint (`/v4/historical/sports/{sport}/events/{eventId}/odds`), never the sport-wide snapshot endpoint (`/v4/historical/sports/{sport}/odds`) — confirmed 10x cost difference per `.planning/research/STACK.md`. This is a hard requirement, not just a cost optimization: using the sport-wide endpoint would blow through the entire month's budget on a fraction of the games.
...
- `.planning/research/STACK.md` — the per-event vs. sport-wide endpoint cost formula (10x difference, HIGH confidence), SQLite archive rationale, confirmed pricing tiers (verified 2026-08-19)
