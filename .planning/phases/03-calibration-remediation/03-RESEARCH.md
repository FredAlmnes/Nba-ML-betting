...
1. **Is isotonic regression still the right calibration method given the actual `calibrate` sample size (172) falls well below sklearn's own ~1000-sample guidance?**
...
2. **Should the split-boundary logic be extracted into a small pure/testable function, or should the regression test (if added) just duplicate the date-cutoff formula inline?**
