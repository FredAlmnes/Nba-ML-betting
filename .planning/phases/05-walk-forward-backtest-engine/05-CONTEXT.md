...
- CLV per bet is computed as the vig-free implied probability at the bet-time snapshot minus the vig-free implied probability at the closing snapshot (both already archived in `odds_arkiv.db` as `snapshot_type` `bet_time`/`closing`), reusing `strategy.fjern_vigorish()` on both — mirrors the vig-removal reasoning already applied to the value calculation, so bookmaker margin never leaks into the CLV signal (BT-06).
